...
export class SteadfastProvider {
  ...
  async bookOrder(cfg: SteadfastConfig, params: SteadfastBookParams) {
    ...
    this.logger.log(
      `Steadfast bookOrder → POST ${this.baseUrl}/create_order body=${JSON.stringify(body)}`,
    );

    try {
      const { data } = await firstValueFrom(
        this.http.post(`${this.baseUrl}/create_order`, body, {
          headers: this.headers(cfg),
        }),
      );

      this.logger.log(`Steadfast bookOrder ← ${JSON.stringify(data)}`);
      ...
      this.logger.error(
        `Steadfast bookOrder FAILED — status=${axiosErr.response?.status} ` +
  ...
  async trackOrder(cfg: SteadfastConfig, consignmentId: string) {
    try {
      ...
      const axiosErr = err as AxiosError;
      this.logger.error(
        `Steadfast trackOrder FAILED — status=${axiosErr.response?.status} ` +
          `body=${JSON.stringify(axiosErr.response?.data)}`,
      );
      throw err;
    }
  ...
  cancelOrder(_cfg: SteadfastConfig, consignmentId: string) {
    this.logger.warn(
      `Steadfast cancel requested for ${consignmentId} — manual cancel required via portal`,
    );
    return {
      cancelled: false,
  ...
  calculateCharge(
    _cfg: SteadfastConfig,
    params: { weight: number; codAmount: number },
  ) {
    const baseCharge = 130;
