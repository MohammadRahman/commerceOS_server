...
//     // ── Validate courier config BEFORE creating any records ──────────────────
//     // Fail fast with a clear 400 rather than silently failing in the outbox worker.
//     const courierRow = await this.orgCouriers.findOne({
//       where: { orgId, type: dto.courierProvider as any },
//     });
...
//     // ── Enqueue outbox job — worker calls courier API asynchronously ──────────
//     // codAmount and weight are forwarded so the worker can pass them to the
//     // courier API without needing to re-fetch the order.
//     await this.outbox.enqueue(orgId, 'shipment.book', {
//       shipmentId: shipment.id,
//       courierProvider: dto.courierProvider,
//       overrides: dto,
