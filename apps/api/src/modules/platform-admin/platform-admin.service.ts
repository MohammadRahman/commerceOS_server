...
// ─── Valid feature flag keys — mirrors PlanFeature in plans.ts ────────────────
// Server-side validation prevents arbitrary keys polluting the JSONB column.
...
export class PlatformAdminService {
  ...
  // ── Update plan ────────────────────────────────────────────────────────────
  // NEW — was missing from the original service.
  // Validates against VALID_PLANS, logs the change with previous value.
  ...
  async updatePlan(
    ...
    if (!VALID_PLANS.has(normalized)) {
      throw new BadRequestException(
        `Invalid plan "${plan}". Must be one of: FREE, PRO, ENTERPRISE.`,
      );
    }
    ...
    const previousPlan = (org as any).plan ?? 'FREE';

    await this.orgs.update({ id: orgId } as any, { plan: normalized } as any);

    this.logger.log(
      `[PlatformAdmin] Plan updated: org=${orgId} ${previousPlan}→${normalized} by admin=${adminUserId}`,
    );
  ...
  // ── Feature flags ──────────────────────────────────────────────────────────
  // FIXED — was merging with existing flags, now replaces entirely.
  // Also validates keys against VALID_FLAG_KEYS and strips unknown ones.
  // Frontend sends the full intended flags object, not a delta.
  ...
  async setFeatureFlags(
    ...
    // Validate and sanitize — strip unknown keys so arbitrary JSONB can't be injected
    const sanitized: Record<string, boolean> = {};
    const ignored: string[] = [];

    for (const [key, val] of Object.entries(flags)) {
      if (VALID_FLAG_KEYS.has(key) && typeof val === 'boolean') {
        sanitized[key] = val;
      } else {
        ignored.push(key);
      }
    ...
    if (ignored.length > 0) {
      this.logger.warn(
        `[PlatformAdmin] setFeatureFlags: unknown keys stripped for org=${orgId}: [${ignored.join(', ')}]`,
      );
    }

    // Full replace — not a merge. Frontend owns the complete flags state.
    await this.orgs.update(
      { id: orgId } as any,
      { featureFlags: sanitized } as any,
    );

    this.logger.log(
      `[PlatformAdmin] Feature flags replaced: org=${orgId} flags=${JSON.stringify(sanitized)}${adminUserId ? ` by admin=${adminUserId}` : ''}`,
    );
  ...
  async impersonate(
    ...
    if (!owner)
      throw new NotFoundException('No owner found for this organization');
    ...
    this.logger.warn(
      `[PlatformAdmin] IMPERSONATION: admin=${adminUserId} → org=${orgId} owner=${owner.id}`,
    );
