...
import { StorefrontService, UpsertStoreDto, CreateProductDto, StorefrontOrderDto } from './storefront.service';
...
export class StorefrontController {
  ...
  getPublicProducts(@Param('slug') slug: string) {
    return this.storefront.getStoreBySlug(slug).then((store) =>
      this.storefront.listProducts(store.orgId, true),
    );
  }
  ...
  getPublicProduct(
    ...
    return this.storefront.getStoreBySlug(slug).then((store) =>
      this.storefront.getProduct(store.orgId, productSlug),
    );
  }
  ...
  async sitemap(@Param('slug') slug: string) {
    ...
    const urls = [
      { loc: `${origin}/`, priority: '1.0', changefreq: 'daily', image: null as string | null, imageTitle: null as string | null },
      ...prods.map((p) => ({
...
Sitemap: ${origin}/sitemap.xml`;
  ...
  @Post('s/:slug/orders')
  async placeOrder(@Param('slug') slug: string, @Body() dto: StorefrontOrderDto) {
    const store = await this.storefront.getStoreBySlug(slug);
