...
export class TaxProfileService {
  ...
  async upsertProfile(
    ...
    Object.assign(profile, {
      ...
      // FIX: defaultVatRate is a decimal column typed as string.
      // TypeORM will coerce a number on write, but we cast explicitly so
      // the TypeScript type aligns with the entity.
      defaultVatRate: String(dto.defaultVatRate ?? 24),
      isSoleTraderFie: dto.isSoleTraderFie ?? false,
      // FIX: was incorrectly set to `dto.isSoleTraderFie` — copy-paste bug.
      // This should come from its own DTO field.
      paysAdvanceIncomeTax: dto.paysAdvanceIncomeTax ?? false,
    });
  ...
  async createEmployee(
    ...
    const employee = this.employeeRepo.create({
      ...
      // hourlyRate is a string decimal in the entity — store as string
      hourlyRate: dto.hourlyRate != null ? String(dto.hourlyRate) : null,
      isActive: true,
    });
  ...
  async deactivateEmployee(id: string, orgId: string): Promise<void> {
    const employee = await this.employeeRepo.findOne({ where: { id, orgId } });
